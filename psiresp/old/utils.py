...
def generate_connolly_spheres(symbols, radii, density=1.0):
    """
...
def generate_connolly_shells(
    ...
    for sf in scale_factors:
        ...
        shell = generate_connolly_spheres(symbols, radii, density=density)
...
def generate_vdw_surface(points, radii, coordinates, rmin=0, rmax=-1):
    ...
    if len(points) != len(coordinates):
        err = ("Length of ``points`` must match length of ``coordinates``"
               "Generate unit shells for atoms with generate_connolly_shells()")
        raise ValueError(err)
    if len(radii) != len(coordinates):
        err = ("Length of ``radii`` must match length of ``coordinates``"
               "Generate scaled radii for atoms with generate_connolly_shells()")
        raise ValueError(err)
