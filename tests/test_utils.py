...
@pytest.mark.fast
@pytest.mark.parametrize('factor,radii_name,scaled', [(1.4, 'msk', [1.68, 2.1, 1.96]), (1.6, 'msk', [1.92, 2.4, 2.24]),
                                                      (1.8, 'msk', [2.16, 2.7, 2.52]), (2.0, 'msk', [2.4, 3., 2.8]),
...
def test_scale_radii(factor, radii_name, scaled):
    radii = utils.scale_radii({'H', 'C', 'O'}, factor, use_radii=radii_name)
...
def test_gen_connolly_spheres(scaled, density, n_points):
    ...
    points, rad = utils.gen_connolly_spheres('CHOHCC', radii, density=density)
    assert len(rad) == 6
    assert len(points) == 6
...
@pytest.mark.parametrize('vdw_radii,scale_factors,density,radii,n_points', [
    ({}, (1.4, 2.0), 1.0, [[1.68, 2.1, 1.96], [2.4, 3., 2.8]], [[35, 55, 48], [72, 113, 98]]),
    ({}, (1.4, 2.0), 2.0, [[1.68, 2.1, 1.96], [2.4, 3., 2.8]], [[70, 110, 96], [144, 226, 197]]),
    ({
        'C': 1
    }, (2.0, ), 2.0, [[2.4, 2, 2.8]], [[144, 100, 197]]),
])
def test_gen_connolly_shells(vdw_radii, scale_factors, density, radii, n_points):
    points = utils.gen_connolly_shells('CHOHCC', vdw_radii=vdw_radii, scale_factors=scale_factors, density=density)
...
def test_resp2_grid_generation(elements, coordfile, gridfile):
    ...
    vdw_point_list = utils.gen_connolly_shells(elements, use_radii="bondi", scale_factors=scale_factors, density=2.5)
